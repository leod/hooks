...
use view::{self, Assets};

// TODO: Lots of code duplication with view::rect! Ideally, we would automatically attach
// `view::rect::Draw` for entities with `wall::Size` components, I think. This isn't easily
// possible right now because we have the ctor stuff only for replicated entities, which walls do
// not need to be.

pub fn register_load(reg: &mut view::Registry) {
    reg.draw_fn(draw);
}
